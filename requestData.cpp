...
priority_queue<mytimer*,deque<mytimer*>,timerCmp> myTimerQueue;				//用双端队列存储的大根堆
...
:now_read_pos(0),
 ...
 :now_read_pos(0),
  ...
  requestData::~requestData(){
	  ...
	  if(timer !=NULL){
		  timer->clearReq();		//request_data = NULL;   this->setDeleted();  等大根堆来删除了
		  timer=NULL;
	  }
	  close(fd);
  }
...
void requestData::seperateTimer()		//就是把timer干掉
{
    if (timer)
    {
        timer->clearReq();
...
void requestData::handleRequest()
{
    ...
    while (true)
    {
        ...
        else if (read_num == 0)
        {
            ...
            if (errno == EAGAIN)
            {
                if (againTimes > AGAIN_MAX_TIMES)		//可以again200次
                    isError = true;
                else
                    ++againTimes;			
            }
        ...
        string now_read(buff, buff + read_num);		//本次读到的内容放进一个string中
        content += now_read;					//并且追加到content中，便于下方进行处理
	
	cout<<"this time i read: "<<now_read<<endl;
        ...
        {
	cout<<"try to analysisRequest"<<endl;
    ...
    if (state == STATE_FINISH)
    {
        ...
        {
            delete this;			//如果没有长连接选项，就直接断开连接退出了，否则再次加入epoll，
            return;
        }
    }
 ...
 int requestData::parse_URI()
{
    ...
    cout<<"request_line find, method = " << method <<endl;
    //printf("method = %d\n", method);
    // filename
    pos = request_line.find("/", pos);		//找到请求资源的开始部分
    if (pos < 0)
    {
        return PARSE_URI_ERROR;
    }
    ...
    {
        int _pos = request_line.find(' ', pos);		//请求资源的结束部分
        if (_pos < 0)
            return PARSE_URI_ERROR;
        else
        {
            if (_pos - pos > 1)
            {
                ...
                int __pos = file_name.find('?');			//?后的是query语句
                if (__pos >= 0)
                {
                    file_name = file_name.substr(0, __pos);		//舍弃query语句
                }
            }
                
            else
                file_name = "index.html";			//请求资源不正确的话，就设为这个
        }
        pos = _pos;
    }
 ...
 int requestData::parse_Headers()
{
        ...
        switch(h_state)
        {
            ...
            case h_CR:
            {
                if (str[i] == '\n')
                {
                    ...
                    headers[key] = value;								//将上面读到的key: value值装进header里，一个map
                    now_read_line_begin = i;
                }
                else
                    return PARSE_HEADER_ERROR;
    ...
    if (h_state == h_end_LF)
    {
        ...
        cout<<"PARSE_HEADER_SUCCESS, headers.size: " << headers.size() <<endl;
 ...
 int requestData::analysisRequest()
{
    ...
    {
        //get content
        char header[MAX_BUFF];								//响应头
        sprintf(header, "HTTP/1.1 %d %s\r\n", 200, "OK");
        if(headers.find("Connection") != headers.end() && headers["Connection"] == "keep-alive")
        {
            keep_alive = true;														//获得长连接信息
        ...
        sprintf(header, "%sContent-length: %zu\r\n", header, strlen(send_content));		//为啥不用string
        sprintf(header, "%s\r\n", header);
        size_t send_len = (size_t)writen(fd, header, strlen(header));			//发送响应头
        ...
        send_len = (size_t)writen(fd, send_content, strlen(send_content));			//发送响应内容
  ...
  //      imwrite("receive.bmp", test);
    ...
    else if (method == METHOD_GET)
    {
        ...
        const char* filetype;
        if (dot_pos < 0) 
            filetype = MimeType::getMime("default").c_str();		//获得请求文件的类型
        else
            filetype = MimeType::getMime(file_name.substr(dot_pos)).c_str();
        struct stat sbuf;
        if (stat(file_name.c_str(), &sbuf) < 0)			//找文件
        {   
...
void requestData::handleError(int fd, int err_num, string short_msg)
{
    ...
    body_buff += "<hr><em> cja's Web Server</em>\n</body></html>";
 ...
 : deleted(false), 
   request_data(_request_data)
   {
	   ...
	   // 以毫秒计
	   expired_time= ((now.tv_sec * 1000) + (now.tv_sec / 1000)) +timeout;
	}
 ...
 mytimer::~mytimer(){
	 ...
	 if(request_data!=NULL){
		 ...
		 delete request_data;			//手动delete，其实可以用智能指针来管理
		 request_data=NULL;
	 }
 }
